...
public abstract class MonthView extends View {
    ...
    protected boolean isHighlighted(int year, int month, int day) {
        DateTime[] highlightedDaysJoda = mController.getHighlightedDays();
        if (highlightedDaysJoda == null) return false;
        Calendar[] highlightedDays = new Calendar[0];
        for (int i = 0; i < highlightedDaysJoda.length; i++) {
            highlightedDays[i] = highlightedDaysJoda[i].toCalendar(mContext.getResources().getConfiguration().locale);
        }
