...
public class CropImageActivity extends AppCompatActivity
    ...
    private boolean mWasCameraCanceled = false;
        ...
        if (savedInstanceState == null) {
            if (mCropImageUri == null || mCropImageUri.equals(Uri.EMPTY)) {
                if (CropImage.isExplicitCameraPermissionRequired(this)) {
                    ...
                    if (mOptions.runCamera) {
                        CropImage.startCameraActivity(this);
                    } else {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CropImage.PICK_IMAGE_CHOOSER_REQUEST_CODE) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                loadImage(CropImage.getPickImageResultUri(this, data));
    ...
    protected void saveImage() {
        mCropImageView.setProgressBarVisibility();
        if (mOptions.noOutputImage) {
            setResult(null, null, 1);
        } else {
            Uri outputUri = getOutputUri();
            mCropImageView.saveStandardImageAsync(
                    outputUri,
                    mOptions.outputCompressFormat,
                    mOptions.outputCompressQuality,
                    mOptions.outputRequestWidth,
    ...
    protected void setResultCancel() {
        ...
        mWasCameraCanceled = true;
    }

    @Override
    protected void onResume() {
        super.onResume();
        if (mWasCameraCanceled) {
            finish();
        }
    }
    ...
    @SuppressLint("NewApi")
    private void loadImage(Uri cropImageUri) {
        mCropImageUri = cropImageUri;
