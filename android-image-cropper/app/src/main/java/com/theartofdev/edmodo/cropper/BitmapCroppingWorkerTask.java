...
final class BitmapCroppingWorkerTask
  ...
  private final boolean mShouldCrop;
    ...
    try {
      if (!isCancelled()) {
        ...
        if (mShouldCrop) {
          ...
          if (mBitmap != null) {
            bitmapSampled = new BitmapUtils.BitmapSampled(mBitmap);
          } else {

              BitmapUtils.RotateBitmapResult rotateResult =
                      BitmapUtils.rotateBitmapByExif(BitmapUtils.decodeSampledBitmap(mContext, mUri, mReqWidth, mReqHeight).bitmap, mContext, mUri);
            bitmapSampled = new BitmapUtils.BitmapSampled(rotateResult.bitmap);
          }
        }
        Bitmap bitmap =
