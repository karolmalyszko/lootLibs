...
def build_cardio()
  CommandProcessor.command("rm -rf app/src/main/libs/* app/src/main/obj/*")
  CommandProcessor.command("./gradlew clean :app:assembleRelease releaseDoc", live_output=true)
end
...
def publish_to_maven(version)
  CommandProcessor.command("./gradlew :app:uploadArchives", live_output=true)
  CommandProcessor.command("./gradlew :app:closeRepository", live_output=true)
  CommandProcessor.command("sleep 60")
  CommandProcessor.command("./gradlew :app:promoteRepository", live_output=true)
end
...
configatron.downstream_repos = [
  DownstreamRepo.new(
    ...
    :full_file_sync => true,
    :files_to_copy => [
      CopyFile.new("app/build/outputs/aar/card.io-release.aar", "card.io-__VERSION__.aar", "aars"),
    ],
    :post_copy_methods => [
...
configatron.doc_files_to_copy = [
  CopyFile.new("app/build/docs/javadoc/release/*", ".", ".")
]
